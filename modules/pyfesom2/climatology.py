...
class climatology(object):
    ...
    Presently options are WOA2018 &  PHC3.0 and GDEM.
    ...
    climname : str
        Name of the climatology ('woa' or 'phc')
    ...
    '''
    def __init__(self, path, climname):
        ...
        if climname=='woa':
            ncfileT = Dataset(path+'woa18_decav_t00_01.nc')
            ncfileS = Dataset(path+'woa18_decav_s00_01.nc')
            self.T = np.squeeze(np.copy(ncfileT.variables['t_an'][:]))
            x=np.copy(ncfileT.variables['lon'][:])
            ind=[i[0] for i in sorted(enumerate(x), key=lambda x:x[1])]
            x=np.sort(x)
            self.x=x
            self.y=ncfileT.variables['lat'][:]
            self.z=ncfileT.variables['depth'][:]
            self.T[:,:,:]=self.T[:,:,ind]
            self.S=np.squeeze(np.copy(ncfileS.variables['s_an'][:]))
            self.S[:,:,:]=self.S[:,:,ind]
            depth3d = np.zeros(self.S.shape)
            for i in range(self.z.shape[0]):
                depth3d[i, :, :] = self.z[i]
