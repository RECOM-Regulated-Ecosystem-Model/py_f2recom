...
def select_depths(uplow, mesh, verbose = False):
...
def areasum_data(data, mesh, mask="Global Ocean"):
    """Calculate volume weighted sum at surface.
    ...
    if mask is None:
        ...
        data_by_area = ((np.ma.masked_equal(data[:,mask],1) * nod_area[0,:].T[mask]).sum(axis=1))
    else:
        data_by_area = ((np.ma.masked_equal(data[:,mask],1) * nod_area[0,:].T[mask]).sum(axis=1))
